...
RegistrationService.proto.testRegistrationPostUrl = function(opts, callback) {
  var method = new RequestMethod(this.configuration_, {
    method: RegistrationService.serviceMethods.testRegistrationPostUrl,
    authtype : opts.authType,
    urlname : opts.urlName,
    urlpass  : opts.urlPass,
  ...
  request.submit(function(r) {
    ...
    if(r.isOk()) {
      ...
      _.each(modules, function(module){
        ...
        if(module.runtime[0].success_status[0] != 'Passed'){
          success = false;
        }
...
RegistrationService.prototype.getFullRegistrationListResults = function(learnerId, enrollments, callback){
  var method = new RequestMethod(this.configuration_, {
    ...
    if(r.isOk()){
      ...
      */
      ...
      _.each(data[0].registration, function(reg){
        ...
        if(learners[reg.learnerId[0]].courses[reg.courseId[0]] === undefined && enrollments.indexOf(reg.registrationId[0]) != -1){
          // initialize learner course
          learners[reg.learnerId[0]].courses[reg.courseId[0]] = {
              courseName: reg.courseTitle[0],
              enrolledTime: moment(reg.createDate[0]).fromNow(),
...
RegistrationService.serviceMethods = {
  ...
  testRegistrationPostUrl : 'rustici.registration.testRegistrationPostUrl'
};

/**
 * Returns the raw data from the registration list results.
 *
 * @param {!Date} after include only registrations after the given date.
 * @param {!Function} callback Callback function.
 *
 */

RegistrationService.prototype.getRawRegistrationListResults = function(after, callback){
  var params = {
    method: RegistrationService.serviceMethods.getRegistrationListResults,
    resultsformat: 'course',
    dataformat: 'xml'
  ...
  var method = new RequestMethod(this.configuration_, params);
